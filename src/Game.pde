...
int currentLevel = 1;                                                  // Level (difficulty increases)
int asteroidHealth = 1;                                                // Determine if asteroid is alive
int score;                                                             // The players score
int specialWeaponNumber = 2;                                           // Number of special weapon bullets at start of game
...
void draw() {
  ...
  if (gameEndTimer > 0) {
    ...
    fill(0);
    textFont(font, 30);
    text("Special Bullets: " + specialWeaponNumber, 20,890);          // Displays the number of special bullets the player currently has
    text("Level: " + currentLevel, 280,890);                          // Displays the current level
    text("Score: " + score, 450,890);                                 // Displays the current score
...
void mousePressed() {
  ...
  if (mouseButton == RIGHT && !gameOver) {
    if (specialWeaponNumber > 0) ShootSpecialWeapon();
  }
}
...
void ShootSpecialWeapon() {
  ...
  specialWeaponNumber --;                                                                              // Increments the number of available special weapons
  stroke(0, 255, 0);
